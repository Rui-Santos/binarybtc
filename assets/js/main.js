...
function displayOptions(displaysymbols) {
  $.each(displaysymbols, function( index, symbol ) {
    ...
    option.push('<div style="margin-top: 15px;" class="panel'+symbol+'">'+
...
var displaysymbols = ['BTCUSD', 'GCJ14.CMX']
...
var publictrades = true;
   socket.on('activetrades', function (data) {
    ...
    $('.tradesbody').html('');
    if (data[0] != null) {
      var tradehtml = '<div class="header">Trades <span class="expiretime" style="float:right;"></span></div>';
    } else {
...
$( ".usertrade" ).each(function( index ) {
      ...
      if (direction == 'Put') {
        if (symbolprice < tradeprice) {
          $('#'+tradeid+'').effect("highlight", {color: 'hsl(113, 100%, 35%, 0.15)'}, 1530, "easeInOutCirc");
